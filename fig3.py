#!/usr/bin/env python3
"""
Figure 3: General opinion of AI in healthcare
Horizontal stacked bar chart showing general opinion levels about AI in healthcare

To run: conda activate eda && python fig3.py
"""
...
def create_ai_opinion_chart():
    """Create horizontal stacked bar chart for AI opinion levels"""
    
    # Read the survey data
    df = pd.read_excel('data/S2_Survey_Data.xlsx', sheet_name='Sheet 1')
    
    # Get AI opinion counts and percentages (excluding missing data)
    opinion_counts = df['ai_use_gen_opinion.q13'].value_counts(dropna=True)
    total = df['ai_use_gen_opinion.q13'].notna().sum()  # Only count non-missing responses
    opinion_pct = (opinion_counts / total * 100).round(1)
    ...
    percentages = [opinion_pct.get(cat, 0) for cat in categories]
    ...
    for i, (category, percentage, color) in enumerate(zip(categories, percentages, colors)):
        ...
        if percentage > 0:
            # Rotate text for very small segments (like "Very unfavorable" at 3.6%)
    ...
    ax.set_title('General opinion of AI in healthcare', 
    ...
    # Save the figure
    plt.savefig('fig3_ai_opinion.png', dpi=300, bbox_inches='tight')
    plt.savefig('fig3_ai_opinion.pdf', bbox_inches='tight')
    
    # Show the figure
    plt.show()
    
    # Print summary statistics
    print("AI General Opinion Distribution:")
    print("=" * 40)
